...
class TDD:
    ...
    def show(self, fname='output', real_label=True):
        ...
        dot.edge('-0', str(self.node.idx), color="blue",
...
def get_count():
    global add_find_time, add_hit_time, cont_find_time, cont_hit_time, node_find_time, node_hit_time
    print("node:", node_hit_time, "/", node_find_time, "/",
          node_hit_time / node_find_time if node_find_time != 0 else 0)
    ...
    print("cont:", cont_hit_time, "/", cont_find_time, "/",
          cont_hit_time / cont_find_time if cont_find_time != 0 else 0)
...
def find_computed_table(item):
    ...
    if item[0] == 's':
        ...
        the_key = (
            '*', get_int_key(item[1].weight), item[1].node, get_int_key(item[2].weight), item[2].node, item[3][0],
            item[3][1], item[4])
        cont_find_time += 1
        if computed_table.__contains__(the_key):
            res = computed_table[the_key]
            ...
            return tdd
        the_key = (
            '*', get_int_key(item[2].weight), item[2].node, get_int_key(item[1].weight), item[1].node, item[3][1],
            item[3][0], item[4])
        if computed_table.__contains__(the_key):
            res = computed_table[the_key]
            tdd = TDD(res[1])
...
def get_measure_prob(tdd):
    ...
    tdd.node.meas_prob[0] = abs(tdd.node.out_weight[0]) ** 2 * sum(tdd.node.successor[0].meas_prob) * 2 ** (
            tdd.node.key - tdd.node.successor[0].key)
    tdd.node.meas_prob[1] = abs(tdd.node.out_weight[1]) ** 2 * sum(tdd.node.successor[1].meas_prob) * 2 ** (
            tdd.node.key - tdd.node.successor[1].key)
    return tdd
